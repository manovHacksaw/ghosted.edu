...
import { useState, useEffect } from "react"
import { useRouter } from "next/navigation"
import { Ghost, ArrowRight, CheckCircle, Loader2 } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@/components/ui/card"
import { Progress } from "@/components/ui/progress"
...
export const ProcessingResults = ({ formData }: { formData: any }) => {
  ...
  // When error occurs, show a loader and redirect to /dashboard after a delay.
  useEffect(() => {
    if (error) {
  ...
  useEffect(() => {
    ...
    async function getInsights() {
      try {
        ...
        // Instead of displaying an error card, we set an error state to trigger redirection.
